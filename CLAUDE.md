...
- **肘（⚠️曾搞反，已实测纠正）**：q **减小=屈肘**（正常弯曲，可到负值，实测
  -0.77 可达）；q 增大=伸直方向（q≈2.3 手臂近乎伸直微反别，视觉"反关节"）。
  自然下垂 q≈1.3。折臂动作（敬礼）用负 q
- **肩yaw**（15左/22右）⚠️特殊电机：**只执行 tau、完全忽略 kp/q 位置指令**
  （实测 kp140 纹丝不动，tau 2.5Nm 立刻动；范围约 -3.0~+1.2）。控制必须用
  **软件PD发tau**（见 yaw_pd_test.py：软kp6/kd0.8/tau限2.5 可平滑伺服）。
  静摩擦大，小幅定位有粘滑（提高软kp/tau上限改善）。直接发大tau会瞬间甩到
  限位（dq可达6.8rad/s），务必闭环+限幅。历史注：敬礼参数里的 yaw 实际从未生效
...
## 当前进度 / 待办（2026-06-03 交接状态）

- ✅ **已验证可用**：`jingli`(敬礼)、`guzhang`(鼓掌·肩开合20s)、`guzhang2`(鼓掌·
  双肩yaw软件PD大开合)——均为肘语义修正后调参，实机确认
- ⚠️ **待修**：bainian/baoquan/heshi/wave/bolang/conductor/huanying ——全部在
  肘语义纠正前调参（肘角理解反了），姿态不对，需按正确肘语义（q小=屈）重调 elbow
- 下一步建议：①重调待修动作 ②record.py 录内置挥手做克隆回放 ③LAFAN1 上肢回放
  ④左肩yaw 复用软件PD方案。详见 README.md 交接文档
